...
class YOLOPAFPN(nn.Module):
    ...
    def __init__(
        self, depth=1.0, width=1.0, in_features=("dark3", "dark4", "dark5", "dark6"),
        in_channels=[256, 512, 768, 1024], depthwise=False, act="relu",
    ):
        super().__init__()
    ...
    def forward(self, input):
        ...
        f_out0 = self.C3_p5(f_out0)  # 1024->512/16

        fpn_out1 = self.reduce_conv1(f_out0)  # 512->256/16
        f_out1 = self.upsample(fpn_out1)  # 256/8
        f_out1 = torch.cat([f_out1, x2], 1)  # 256->512/8
        pan_out3 = self.C3_p4(f_out1)  # 512->256/8


        fpn_out2 = self.reduce_conv2(f_out1)  # 512->256/16
        f_out2 = self.upsample(fpn_out2)  # 256/8
        f_out2 = torch.cat([f_out2, x3], 1)  # 256->512/8
        pan_out2 = self.C3_p3(f_out2)  # 512->256/8

        p_out1 = self.bu_conv2(pan_out2)  # 256->256/16
        p_out1 = torch.cat([p_out1, fpn_out2], 1)  # 256->512/16
